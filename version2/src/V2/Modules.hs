...
module V2.Modules (getModules, ModuleInfo (..), storeCore) where
...
import V2.Core.Parser (parseCoreFile)
import V2.Core.PrettyPrint ()
import V2.Core.Syntax (Core)
import V2.Parser (parseModuleFile, parseModuleImports)
import V2.PrettyPrint (Disp (disp), render)
import V2.Syntax (MName, ModuleImport (..), UModule (..))
...
storeCore name core = liftIO $ writeFile fixedName (render $ disp core)
  where
    fixedName = dropExtension name <.> "v2core"
...
reparse useCache top (ModuleInfo _ fileName _) = TimeStats.measureM ("Loading: " ++ fileName) $ do
  b <- liftIO $ doesFileExist (dropExtension fileName <.> "v2core")
