module V2.ModuleTypeChecker (tcModule, verifyCore) where
...
import V2.PrettyPrint (D (..), Disp (disp), render)
import V2.Syntax (Decl (..), Module (..), Sig (Sig), TName, TeleScope, Term, Type)
import V2.TcEnvironment.ModuleEnvironment (TcMonad, emptyState)
import qualified V2.TcEnvironment.ModuleEnvironment as Env
import qualified V2.TcEnvironment.TermEnvironment as Env
import V2.TermTypeChecker (checkType, tcType)
...
tcDecl (ModApply m1 params m2 args) =
  TimeStats.measureM "TypeCheck: ModuleAliases" $
    TimeStats.measureM ("TCAlias:" ++ render (disp m1)) $
      tcTeleScope
        params
        ( do
