...
import qualified V2.LayoutToken as Token
import V2.Syntax
...
applyDef = do
  ...
  path <- modulePath
  args <- many factor
  return $ UModApply modName $ Unbound.bind (Unbound.rec param) (map (,[]) path, args)
...
modulePathT :: LParser ModPathT
modulePathT = do
  h <- p
  t <- many (dot >> p)
  return $ h : t
  where
    p = ((,[]) <$> identifier) <|> parens (do n <- identifier; (n,) <$> many factor)
...
factor = do
  ...
  choice
    [ lambda <?> "a lambda",
      ...
      (if b then try varParserT else varParser) <?> "a var",
...
varParser = do
  f <- identifier
  t <- many (dot >> identifier)
  if null t then return $ Var $ Unbound.string2Name f else return $ QualifiedTerm $ toQualifiedT $ h $ f : t
  where
    h [x] = Current $ Unbound.string2Name x
    h (x : xs) = Q x $ h xs
    h [] = undefined
...
qualifiedVar = QualifiedTerm . toQualifiedT <$> qualifiedName

varParserT :: LParser Term
varParserT = do
  f <- p
  (dot >> (QualifiedTerm . uncurry QT f <$> parseQ)) <|> (case f of (z, []) -> return (Var $ Unbound.string2Name z); _ -> fail "failed to parse")
  where
    p = ((,[]) <$> identifier) <|> parens (do n <- identifier; (n,) <$> many factor)
    parseQ = do
      f <- p
      (dot >> (uncurry QT f <$> parseQ)) <|> (case f of (z, []) -> return (CurrentT $ Unbound.string2Name z); _ -> fail "Arguments to last name of qualified term")
