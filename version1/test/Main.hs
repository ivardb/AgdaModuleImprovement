...
import V1.Error (runErrorMonad)
import V1.ModuleTypeChecker (verifyCore)
import V1.Modules (getModules)
import V1.PrettyPrint (Disp (disp), render)
import V1.TypeChecking (Config (..), typeCheck)
  ...
  testCase (takeBaseName name) $ do
    ...
    when
      ...
      ( do
          ...
          ve <- runErrorMonad $ verifyCore [] parsedCore
