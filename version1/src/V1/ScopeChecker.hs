module V1.ScopeChecker (scopeCheckModule) where
...
import V1.Error (Err (..), ErrorMonad, SourceLocation (..))
import qualified V1.Error as Error (err)
import V1.PrettyPrint (D (..), Disp, SourcePos, disp, render)
import V1.Syntax
  ( Decl (..),
    MName,
    ModPath,
...
checkImport (ModuleImport mname) = TimeStats.measureM ("ScopeChecking: Importing " ++ render (disp mname)) $ do
  ...
  case x of
    ...
    Just (Core n _ _ decls) -> do
      modify (\(sc : t) -> addMName [n] [(n, [])] sc : t)
      mapM_ extendDecls decls
  where
    extendDecls (CoreTypeSig s) = modify (\(sc : t) -> addTName (sigName s) (toQualifiedT $ sigName s) sc : t)
    extendDecls (CoreModule n _ decls) = do
...
checkImports = TimeStats.measureM "ScopeChecking: Total Imports" . do mapM_ checkImport
