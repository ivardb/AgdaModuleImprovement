...
module V1.Modules (getModules, ModuleInfo (..), storeCore) where
...
import V1.Core.Parser (parseCoreFile)
import V1.Core.PrettyPrint ()
import V1.Core.Syntax (Core)
import V1.Parser (parseModuleFile, parseModuleImports)
import V1.PrettyPrint (Disp (disp), render)
import V1.Syntax (MName, ModuleImport (..), UModule (..))

-- | getModules starts with a top-level module, and gathers all of the module's
-- transitive dependencies. It returns the list of parsed modules, with all
...
storeCore name core = liftIO $ writeFile fixedName (render $ disp core)
  where
    fixedName = dropExtension name <.> "v1core"
...
reparse useCache top (ModuleInfo _ fileName _) = TimeStats.measureM ("Loading: " ++ fileName) $ do
  b <- liftIO $ doesFileExist (dropExtension fileName <.> "v1core")
