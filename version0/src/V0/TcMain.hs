module V0.TcMain (goFilename) where
...
import V0.Parser (parseExpr)
import V0.PrettyPrint (Disp (..), render)
import V0.TcEnvironment.ModuleEnvironment (emptyState, runTcMonadState)
import V0.TermTypeChecker (inferType)
