...
module V0.Modules (getModules, ModuleInfo (..), storeCore) where
...
import V0.Parser (parseModuleFile, parseModuleImports)
import V0.PrettyPrint (Disp (disp), render)
import V0.Syntax (MName, ModuleImport (..), UModule (..))
...
storeCore name core = liftIO $ writeFile fixedName (render $ disp core)
  where
    fixedName = dropExtension name <.> "v0core"
...
reparse useCache top (ModuleInfo _ fileName _) = TimeStats.measureM ("Loading: " ++ fileName) $ do
  b <- liftIO $ doesFileExist (dropExtension fileName <.> "v0core")
