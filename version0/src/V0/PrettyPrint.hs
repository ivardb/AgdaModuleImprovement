...
-- | A Pretty Printer.
module V0.PrettyPrint (Disp (..), D (..), SourcePos, PP.Doc, PP.render, Display (..), DispInfo (..), defaultDI, withIndent) where
    ...
    Unbound.lunbind bind $ \(ts, (m2, args)) -> do
      ...
      return $
        PP.text (replicate i ' ')
          <> PP.text "module "
          <> disp n
          <> (disp (Unbound.unrec ts) <+> PP.text "=" <+> disp m2 <+> da)

dispModArgs :: [Term] -> DispInfo -> Doc
dispModArgs args = do
  foldrM h (PP.text "") args
  where
    h t d | isLiteral t = do
      dt <- withPrec levelApp (display t)
      return $ dt <+> d
    h t d = do
      dt <- withPrec levelApp (display t)
      return $ (PP.text "(" <> dt <> PP.text ")") <+> d
    isLiteral TyBool = True
    isLiteral TyUnit = True
    isLiteral Type = True
    isLiteral Unit = True
    isLiteral (LitBool _) = True
    isLiteral (Var _) = True
    isLiteral (QualifiedTerm _) = True
    isLiteral _ = False
