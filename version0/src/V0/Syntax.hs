...
-- | Qualified names. Used to reference definitions
...
data UDecl
  ...
  | -- | A module alias
    UModApply MName (Unbound.Bind (Unbound.Rec UTeleScope) (ModPath, [Term]))
  deriving (Show, Generic, Typeable, NFData)
  deriving anyclass (Unbound.Alpha, Unbound.Subst Term)
...
data Decl
  ...
  | -- | A module alias
    ModApply ModPath TeleScope ModPath [Term]
  deriving (Show, Generic, Typeable, NFData)
  deriving anyclass (Unbound.Alpha, Unbound.Subst Term)
...
class ModQualified a where
  ...
  toModPath :: a -> ModPath
  toModPath m = case pop m of
    Nothing -> []
    Just (n, t) -> n : toModPath t
