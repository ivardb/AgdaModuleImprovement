...
module V3.Syntax where

import Data.Function (on)
import Data.Maybe (fromMaybe, listToMaybe)
import Data.Typeable (Typeable)
import GHC.Generics (Generic (..), from)
import Text.ParserCombinators.Parsec.Pos (SourcePos, initialPos, newPos, sourceName, sourceColumn, sourceLine)
...
-- | A Module has a name and a body. The body is used so we can bind the module arguments using unbound
...
-- while we do want to use those for performance. So we switch to Decl after scopechecking
data UDecl
...
-----------------------------------------
-- Scopechecker inserts qualified names and removes unbound bindings to insert unique names.
-- This can be done already as these terms are never substituted into another expression.
-- Any substitions of module parameters will be done on only part of the declarations
-- and are thus done without using the bindings as well.
-----------------------------------------

-- | Module with fully qualified names and without unbound at declaration level
-- TODO: maybe use Unbound better
-- So bind an entire list at once
-- Also use binds in the context as substitutions are probably lazy
