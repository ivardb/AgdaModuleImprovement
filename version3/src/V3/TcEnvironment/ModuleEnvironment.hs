...
module V3.TcEnvironment.ModuleEnvironment
  ( TcMonad,
    runTcMonad,
    runTcMonadState,
    State (..),
...
import Control.Monad.Reader (MonadReader (ask, local), ReaderT (runReaderT), lift)
...
import V3.Error (Err, ErrorMonad)
import qualified V3.Error as Error (err)
import V3.PrettyPrint (D (..), Disp (..), render)
import V3.Syntax
  ( Decl (..),
    MName,
    ModPath,
    ...
    Term (..),
    Type,
    baseName, ModPathT, QualifiedT, splitT, Qualified, split
  )
import V3.TcEnvironment.TermEnvironment (Env, emptyEnv, err)
import Data.Map (Map)
import qualified Data.Map as Map

type TcMonad = StateT State (ReaderT Env ErrorMonad)

instance {-# OVERLAPPABLE #-} MonadReader a m => MonadReader a (ReaderT b m) where
  ask :: MonadReader a m => ReaderT b m a
  ask = lift ask

  local :: MonadReader a m => (a -> a) -> ReaderT b m a1 -> ReaderT b m a1
  local f mx = do
    b <- ask
...
-- | Lookup the second path starting at the root.
