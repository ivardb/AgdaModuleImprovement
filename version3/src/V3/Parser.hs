...
import qualified V3.LayoutToken as Token
import V3.Syntax
...
parseModuleImports name = do
  contents <- liftIO $ readFile name
  liftError $ Unbound.runFreshM $ runParserT (do whiteSpace; moduleImports) [] name contents
...
var = try qualifiedT <|> (Var <$> variable)

qualifiedT :: LParser Term
qualifiedT = do
  f <- many1 (try $ do x <- p; dot; return x)
  t <- identifier
  return $ QualifiedTerm $ h $ f ++ [(t, [])]
  where
    h [(x, [])] = CurrentT $ Unbound.string2Name x
    h ((x, t) : xs) = QT x t $ h xs
    h _ = undefined
    p = ((,[]) <$> identifier) <|> parens (do n <- identifier; (n,) <$> many factor)
