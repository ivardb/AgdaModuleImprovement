module V3.ScopeChecker (scopeCheckModule) where
...
import V3.Error (Err (..), ErrorMonad, SourceLocation (..))
import qualified V3.Error as Error (err)
import V3.PrettyPrint (D (..), Disp, SourcePos, disp, render)
import V3.Syntax
  ( Decl (..),
    MName,
    ModPath,
    ...
    Term (..),
    UDecl (..),
    UModule (..), UTeleScope, QualifiedT (..), ModPathT, ModQualifiedT (..), toQualified, splitT, toQualifiedT,
  )
